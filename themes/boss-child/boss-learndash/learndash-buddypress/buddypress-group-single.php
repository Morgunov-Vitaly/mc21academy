...
<?php
    ...
    if($group_status) {
      ...
      /* mvv  if(is_active_sidebar('learndash-course') || is_active_sidebar('group')) { */
        if(is_active_sidebar('group')) {    
...
?>
    <!-- mvv <?php echo 'is_active group: ' . is_active_sidebar('group'); echo ' is_active course: ' . is_active_sidebar('learndash-course'); ?> -->
            ...
            <div id="buddypress">
                ...
                <section class="course-header">
                    <div class="table top">
                        ...
                        <div class="table-cell content">
                            <span><?php
                            /* mvv
                            echo LearnDash_Custom_Label::get_label( 'course' ); 
                            */
                            echo __('Группа курса','mv-xxi-translate') . ': '; 
                            ?></span>
                            <header>
                                <h1><?php the_title(); ?></h1>
                            </header>
                ...
                <section id="course-details">
                    <span class="course-statistic">
                        ...
                        if ($total_lessons) {
                            /* mvv */
                            echo __('Кол-во уроков курса' ,'mv-xxi-translate') . ': ' . number_format_i18n($total_lessons); 
                        }
    ...
    <?php 
    /* Здесь можно отключить вывод одного из сайдбаров -  например,  сайдбар курса здесь лишний */
        /* if (is_active_sidebar('learndash-course') || is_active_sidebar('group') ): */
        if (is_active_sidebar('group') ):
    ?>
        <!-- default WordPress sidebar  <?php echo 'mvv group_status: ' . $group_status; ?> -->
        <div id="secondary" class="widget-area" role="complementary">
            ...
            setup_postdata( $post );
            /* dynamic_sidebar('learndash-course'); mvv */
            wp_reset_query();
            //restore the oringal $groups_template before sidebar.
            $groups_template = $groups_template_safe;

            /*-- Group Sidebar -------------------------*/
            dynamic_sidebar('group'); /* после dynamic_sidebar('learndash-course') выводится сайдбар Group - Single Group  */
            ?>
        </div><!-- #secondary -->
    <?php endif; ?>
