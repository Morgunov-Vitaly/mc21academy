...
function mv_email_admin_plugin_settings() {
    ...
    /* Регистрируем текстовую область с текстом сообщения уведомления 1 (вставляется вначале письма - до вывода блока с данными пользователя) */
    add_settings_field(
        ...
        /* Регистрируем текстовую область с текстом сообщения уведомления 2 (вставляется вконце письма - после блока с данными пользователя) */
    add_settings_field(
            'mv_mv_message_2', 'Текст сообщения администратору (после данных пользователя)', 'fill_mv_message_field_2', 'mv_email_admin_page', 'mv_section_id'
    );
    ...
    /**
     * Заполняем поле mv_mv_message  - текста уведомления №1 вначале
     */
    function fill_mv_message_field() {
        ...
        <p><textarea rows="10" cols="50" name="mv_email_admin_option[mv_message]"><?php echo esc_attr($val) ?></textarea></p>	
        <?php
    }

        
    /**
     * Заполняем поле mv_mv_message  - текста уведомления №2 вконце
     */
    function fill_mv_message_field_2() {
        $val = get_option('mv_email_admin_option');
        $val = $val ? $val['mv_message_2'] : null;
        ?>
        <p><b>Текст письма уведомления о регистрации пользователя:</b></p>
        <p><textarea rows="7" cols="50" name="mv_email_admin_option[mv_message_2]"><?php echo esc_attr($val) ?></textarea></p>	
        <?php
    }
...
function wp_new_user_notification($user_id, $deprecated = null, $notify = '') {
    ...
    if ('user' !== $notify) { // если уведомлять не только пользователя
        ...
        $message_2 = $mv_options['mv_message_2'] ? $mv_options['mv_message_2'] : sprintf(__('Generated by mv-email-admin'), $blogname) . "\r\n\r\n";  //Текст сообщения 1
        ...
        /* Добавляем заключительную часть сообщения */
        $message .= $message_2;
